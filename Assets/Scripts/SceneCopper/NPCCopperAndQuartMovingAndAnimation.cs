...
public class NPCCopperAndQuartMovingAndAnimation : MonoBehaviour
{
    ...
    private void HitClosetStone()
    {
        ...
        if (rand == 1)
        {
            stones = GameObject.FindGameObjectsWithTag("Copper");
        }
        else
        {
            stones = GameObject.FindGameObjectsWithTag("Quart");
        }
        // have 1 prototype stone in scene for instantiate so ignore the this stone
        if (stones.Length == 1)
        {
            stones = rand == 1 ? stones = GameObject.FindGameObjectsWithTag("Quart") : stones = GameObject.FindGameObjectsWithTag("Copper");
            if (stones.Length == 1)
            {
                return;
            }
        ...
        {
            minimumDistanceStone?.GetComponent<HitCopperOreManager>().HitStone(5);
            minimumDistanceStone?.GetComponent<HitQuartOreManager>().HitStone(5);
